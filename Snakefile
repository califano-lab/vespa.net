...
rule all:
    input:
        ...
        "results/pcmeta_substrate_site_regulon.rds", "results/pcmeta_substrate_redundantsite_regulon.rds", "results/pcmeta_substrate_protein_regulon.rds", "results/pcmeta_activity_site_regulon.rds", "results/pcmeta_activity_redundantsite_regulon.rds", "results/pcmeta_activity_protein_regulon.rds"
...
rule pc_substrate_regulon_bs:
    ...
    output:
        iteration = temp("results/{dsid}/pc_substrate_regulon/{seed}/{seed}")
    threads: 1
    singularity:
        "aracne.simg"
    shell:
...
rule pc_substrate_proteinregulon_patch:
    input:
        iteration = "results/{dsid}/pc_substrate_regulon/{seed}/{seed}",
        peptides = rules.prepare_substrate_regulon.output.peptides
    output:
        sitenetwork = "results/{dsid}/pc_substrate_siteregulon/bootstrapNetwork_{seed}.txt",
        proteinnetwork = "results/{dsid}/pc_substrate_proteinregulon/bootstrapNetwork_{seed}.txt"
    threads: 1
    singularity:
        "vespa.simg"
    script:
...
rule pc_substrate_siteregulon_consolidate:
    input:
        iteration = expand("results/{{dsid}}/pc_substrate_siteregulon/bootstrapNetwork_{seed}.txt", seed=seed)
    output:
        network = "results/{dsid}/pc_substrate_siteregulon/network.txt"
    threads: 1
    singularity:
        "aracne.simg"
    shell:
...
rule pc_substrate_proteinregulon_consolidate:
    input:
        iteration = expand("results/{{dsid}}/pc_substrate_proteinregulon/bootstrapNetwork_{seed}.txt", seed=seed)
    output:
        network = "results/{dsid}/pc_substrate_proteinregulon/network.txt"
    threads: 1
    singularity:
        "aracne.simg"
    shell:
...
rule pc_substrate_siteregulon_generate:
    input:
        network = rules.pc_substrate_siteregulon_consolidate.output.network,
        matrix = rules.prepare_substrate_regulon.output.matrix,
        peptides = rules.prepare_substrate_regulon.output.peptides
    output:
        regulon = "results/{dsid}/pc_substrate_siteregulon.rds"
    params:
        proteinlevel = False
    singularity:
        "vespa.simg"
...
rule pc_substrate_proteinregulon_generate:
    input:
        network = rules.pc_substrate_proteinregulon_consolidate.output.network,
        matrix = rules.prepare_substrate_regulon.output.matrix,
        peptides = rules.prepare_substrate_regulon.output.peptides
    output:
        regulon = "results/{dsid}/pc_substrate_proteinregulon.rds"
    params:
        proteinlevel = True
    singularity:
        "vespa.simg"
...
rule pcmeta_substrate_regulon_generate:
    input:
        ref = "reference.rds",
        substrate_regulons = [],
        siteregulons = expand("results/{dsid}/pc_substrate_siteregulon.rds", dsid=dsids),
        proteinregulons = expand("results/{dsid}/pc_substrate_proteinregulon.rds", dsid=dsids)
    output:
...
rule pc_activity_regulon_bs:
    ...
    output:
        iteration = temp("results/{dsid}/pc_activity_regulon/{seed}/{seed}")
    threads: 1
    singularity:
        "aracne.simg"
    shell:
...
rule pc_activity_proteinregulon_patch:
    input:
        iteration = "results/{dsid}/pc_activity_regulon/{seed}/{seed}",
        peptides = rules.prepare_pc_activity_regulon.output.peptides
    output:
        sitenetwork = "results/{dsid}/pc_activity_siteregulon/bootstrapNetwork_{seed}.txt",
        proteinnetwork = "results/{dsid}/pc_activity_proteinregulon/bootstrapNetwork_{seed}.txt"
    threads: 1
    singularity:
        "vespa.simg"
    script:
...
rule pc_activity_siteregulon_consolidate:
    input:
        iteration = expand("results/{{dsid}}/pc_activity_siteregulon/bootstrapNetwork_{seed}.txt", seed=seed)
    output:
        network = "results/{dsid}/pc_activity_siteregulon/network.txt"
    threads: 1
    singularity:
        "aracne.simg"
    shell:
...
rule pc_activity_proteinregulon_consolidate:
    input:
        iteration = expand("results/{{dsid}}/pc_activity_proteinregulon/bootstrapNetwork_{seed}.txt", seed=seed)
    output:
        network = "results/{dsid}/pc_activity_proteinregulon/network.txt"
    threads: 1
    singularity:
        "aracne.simg"
    shell:
...
rule pc_activity_siteregulon_generate:
    input:
        network = rules.pc_activity_siteregulon_consolidate.output.network,
        matrix = rules.prepare_pc_activity_regulon.output.matrix,
        peptides = rules.prepare_pc_activity_regulon.output.peptides
    output:
        regulon = "results/{dsid}/pc_activity_siteregulon.rds"
    params:
        proteinlevel = False
    singularity:
        "vespa.simg"
...
rule pc_activity_proteinregulon_generate:
    input:
        network = rules.pc_activity_proteinregulon_consolidate.output.network,
        matrix = rules.prepare_pc_activity_regulon.output.matrix,
        peptides = rules.prepare_pc_activity_regulon.output.peptides
    output:
        regulon = "results/{dsid}/pc_activity_proteinregulon.rds"
    params:
        proteinlevel = True
    singularity:
        "vespa.simg"
...
rule pcmeta_activity_regulon_generate:
    input:
        ...
        siteregulons = expand("results/{dsid}/pc_activity_siteregulon.rds", dsid=dsids),
        proteinregulons = expand("results/{dsid}/pc_activity_proteinregulon.rds", dsid=dsids),
        fasta = "library.fasta"
    output:
